...
- Interfaces with an **OCB350 phase sensor** for flow-phase monitoring.
...
This ensures the physical injection is **time‑aligned** with UPLC data acquisition, giving reproducible retention times and peak shapes.
...
STLs: `LC_EB.stl`, `LC_EBL.stl` (see `docs/` or `hardware/` folder in repo)

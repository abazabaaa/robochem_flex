...
def errcheck(retval, function, arguments):
    if retval < 0:
        raise RuntimeError(
            "Error {0} occured during {1}".format(retval, inspect.stack()[1][3])
        )
    return arguments
...
def AVS_SetParameter(handle, deviceconfig):
    ...
    temp = struct.pack(
        ...
        + "HBf4096f"  # Irradiance
        + "4096f"  # Reflectance
        + "?HHfIIBBHBBBBBHIIfHH12B"  # SpectrumCorrect
        + "5f5f5f"  # StandAlone
        + "?f2f"  # Temperature
        + "2f2f10f10f "  # TecControl
        + "IIIBHB"  # ProcessControl
        + "9720B"  # EthernetSettings
        + "4096B",  # Reserved  # OemData
        deviceconfig.m_Len,

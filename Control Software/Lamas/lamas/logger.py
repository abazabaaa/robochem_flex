...
class Logger:
    ...
    def start_logging_thread(
        ...
        cls._logs_directory = join(getcwd(), "log", cls._platform)
    ...
    def _message_to_tokens(cls, **kwargs) -> list[str]:
        ...
        return [priority + timestamp + "[", origin, "] " + indentation + message]
    ...
    def _log_message(cls, **kwargs) -> None:
        ...
        if cls.gui_log_requests is not None:
            if not cls.gui_log_requests.full():
                cls.gui_log_requests.put(cls._gui_tokens(message_tokens, **kwargs))
    ...
    def log_message(
        ...
        if isinstance(message, Exception):
            ...
            if len(message.args) >= 1:
                text_message += ": " + message.args[0]
            if hasattr(message, "device"):
